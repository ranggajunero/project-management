  ...
  const submitQaResult = async (projectId, resultAction) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { result: resultAction, task_id: bugForm.taskId, note: bugForm.note },
                  ...
                  <div key={proj.project_id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="p-5 border-b border-slate-100">
                      ...
                      <div className="space-y-2">
                        {proj.tasks.map(t => (
                          <div key={t.task_id} className="text-xs flex justify-between p-2 bg-slate-50 rounded border border-slate-100">
                      ...
                      {bugForm.projectId === proj.project_id && (
                        <div className="mt-4 pt-4 border-t border-slate-200 space-y-3">
                          ...
                          <div className="flex justify-end gap-2">
                            <button onClick={() => setBugForm({ projectId: null, taskId: '', note: '' })} className="px-3 py-1.5 bg-slate-200 text-slate-600 font-bold text-xs rounded">Batal</button>
