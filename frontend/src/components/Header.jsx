export default function Header({ currentRole, activeTab, setActiveTab, onDownloadReport, onLogout }) {
  ...
  const renderTitle = () => {
    if (isAdmin) return <span className="text-purple-700 font-bold">ADMIN PANEL</span>;
    if (isManager) return <span className="text-blue-700 font-bold">MANAGER PANEL</span>;
    if (isWorker) return <span className="text-blue-800 font-bold">WORKER PORTAL</span>;
    if (isClient) return <span className="text-slate-900 font-bold">CLIENT PORTAL</span>;
  };
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex space-x-2">
              ...
              {isWorker && (
                <span className="px-3 py-2 rounded-lg text-sm font-bold bg-blue-50 text-blue-700">
                  Tugas Saya
                </span>
              )}
          ...
          <div className="flex items-center gap-3">
            ...
            {/* fungsi Cetak PDF  */}
            {isManagerOrAdmin && activeTab !== 'profile' && (
              <button onClick={onDownloadReport} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold text-xs shadow-sm transition">
                Cetak PDF
              </button>
            )}
