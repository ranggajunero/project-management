        ...
        <div key={task.task_id} className={`bg-white rounded-xl border p-5 flex flex-col shadow-sm transition ${task.is_locked ? 'border-slate-200 opacity-60 bg-slate-50' : 'border-slate-200 hover:shadow-md'}`}>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100">
            {task.is_locked ? (
              ...
              <button onClick={() => updateTaskStatus(task.task_id, 'review')} className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition shadow-sm">
                Kirim Hasil untuk Direview
              </button>
            ) : task.status === 'revision' ? (
              <div className="space-y-3">
                <div className="p-3 bg-rose-50 border border-rose-100 rounded-lg">
                  <p className="text-[10px] font-bold text-rose-800 uppercase mb-1">Catatan Revisi Manager:</p>
                  <p className="text-xs text-rose-600 font-medium">{task.approvals?.[task.approvals.length - 1]?.note || "Perbaiki data"}</p>
                </div>
                <button onClick={() => updateTaskStatus(task.task_id, 'review')} className="w-full py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-xs font-bold transition shadow-sm">
